...
class DoublePeriodStrategy(OrderTemplate):
    ...
    #     if self.symbol[:2] == 'IF':
    #         lot = int((capital*0.07)/(bar.close*300*0.1))
    #     elif self.symbol[:2] == 'IC':
    #         lot = int((capital*0.07)/(bar.close*200*0.12))
    #     else:
    ...
    def on5MinBar(self, bar):
        ...
        if self.tradeTime(bar):
            ...
            self.lot = int(10000000/(bar.close*20)*0.7*0.2)
            self.takeProfitLot = max(int(self.paraDict["takeProfitLotRatio"]*self.lot), 0) # 止盈单下单量
            self.strategy(bar)
