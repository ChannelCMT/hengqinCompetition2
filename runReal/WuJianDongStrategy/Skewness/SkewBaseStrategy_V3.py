...
class SkewBaseStrategy(OrderTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        self.chartLog = {
                            'datetime':[],
                            'HigherAfterEntry':[None],
                            'LowerAfterEntry':[None],
                            # 'envMa':[],
                            'volumeUpper':[],
                            'volume':[],
                            'skew':[],
    ...
    def on5MinBar(self, bar):
        # 必须继承父类方法
        super().onBar(bar)
        self.lot = int(10000000/(bar.close*30)*0.7*0.2)
        # 交易时段执行
        if self.tradeTime(bar):
            ...
            # 非交易时段，画图信号取空值或保持原来信号
            self.chartLog['datetime'].append(bar.datetime)
            self.chartLog['LowerAfterEntry'].append(self.chartLog['LowerAfterEntry'][-1])
            self.chartLog['HigherAfterEntry'].append(self.chartLog['HigherAfterEntry'][-1])
            # self.chartLog['envMa'].append(None)
            self.chartLog['volumeUpper'].append(None)
            self.chartLog['volume'].append(None)
            self.chartLog['skew'].append(None)
    ...
    def entrySignal(self, bar, envPeriod, signalPeriod):
        ...
        if arrayPrepared1 and arrayPrepared2:
            ...
            volumeSpike, volumeUpper = self.algorithm.volumeSignal(amEnv, self.paraDict)
            ...
            self.chartLog['volume'].append(amSignal.volume[-1])
            self.chartLog['skew'].append(skew[-1])
            self.chartLog['close'].append(bar.close)
            # self.chartLog['skewThreshold_left'].append(None)
            # self.chartLog['skewThreshold_right'].append(None)
        return entrySignal
